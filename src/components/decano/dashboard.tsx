...
import { fetchDecanoStats, fetchDepartamentos, type DecanoStats } from './request';
...
export function DecanoDashboard() {
  ...
  const [departamentos, setDepartamentos] = useState<Array<{ name: string; docentes: number; planes: number; cumplimiento: number }>>([]);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([statsData, deptData]) => {
        setStats(statsData);
        setDepartamentos(deptData.map(d => ({
          name: d.nombre,
          docentes: d.docentes,
          planes: d.planes,
              ...
              {isLoading ? (
                ...
                <>
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-semibold text-(--santoto-primary)">{stats.totalDocentes}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-slate-800 font-medium">Planes Completados</span>
                    <span className="text-sm font-semibold text-(--santoto-primary)">{stats.planesCompletados}/{stats.totalPlanes}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-slate-800 font-medium">Calidad Promedio</span>
                    <span className="text-sm font-semibold text-(--santoto-primary)">{(stats.tasaCumplimiento / 20).toFixed(1)}/5.0</span>
                  </div>
                </>
              ) : (
                <p className="text-sm text-slate-600 text-center">No hay datos disponibles</p>
              )}
            </div>
          </div>
        </div>
